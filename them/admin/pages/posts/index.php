            ...
            <tbody>
                ...
                foreach ($post as $c) { ?>
                    <tr>
                        ...
                        <td><?=$c['w'] ?></td>
                        <td><a class="<?=$c['status'] == 1 ? 'btn btn-danger' : 'btn btn-success' ?>" href="<?=assets('change/status/'.$c['id']) ?>"><?=$c['status'] == 1 ? 'غیرفعال کن' : 'فعال کن' ?></a></td>
                        <td><a class="btn btn-info" href="<?= assets('admin/posts/update/'.$c['id']) ?>">اپدیت</a><a class="btn btn-danger" href="<?= assets('admin/posts/deleted/'.$c['id']) ?>">حذف</a></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
